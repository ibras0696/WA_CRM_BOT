...
from crm_bot.core.models import UserRole


def test_adjust_balance_as_admin(session, admin_user, worker_user):
    shifts.open_shift(worker_user, 100, session=session)
    admin_service.adjust_worker_balance(admin_user, worker_user.phone, -20, session=session)
